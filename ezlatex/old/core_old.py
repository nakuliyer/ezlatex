...
#
# from .errors import *
# from .utils import _rand_filename, is_float_basic
#
#
# def is_float(s):
...
#
#
# def deriv_evalf(expr_u, expr_v, n, first=True):
...
#                     return self.evalf(float(self.value))
#                 raise TypeError("Cannot convert \"{}\" to float".format(self.value))
#                 # best_name = self.name if self.name is not None else self.value
#                 # if best_name in kwargs and is_float(kwargs[best_name]):
#                 #     return self.evalf(kwargs[best_name])
#                 # raise TypeError("Need kwarg \"{}\" to evaluate".format(best_name))
#             elif isinstance(self.value, exp):
#                 exp_value = self.value(*args, **kwargs)
#                 if is_float(exp_value):
#                     return self.evalf(float(exp_value))
...
#                     return self.evalf(self.value)
#                 except TypeError:
#                     # TODO: most of these params are not necessary for new_self to be fine
#                     new_self = exp(value=self.value, method=self.method, mtype=self.mtype, is_base=self.is_base, evalf=self.evalf, name=best_name, ignores_vars=self.ignores_vars)
#                     if isinstance(best_name, str):
#                         warnings.warn("Need kwarg \"{}\" to evaluate fully".format(best_name))
#                     return new_self
#                     # raise TypeError("Need kwarg \"{}\" to evaluate".format(best_name))
...
#                             else:
#                                 # TODO: most of these params are not necessary for new_self to be fine
#                                 new_arg_exp = exp(value=arg_exp.value, method=arg_exp.method, mtype=arg_exp.mtype,
#                                                is_base=arg_exp.is_base, evalf=arg_exp.evalf, name=best_name,
#                                                ignores_vars=arg_exp.ignores_vars)
#                                 args.append(new_arg_exp)
#                                 bad_kwargs.append(best_name)
#                                 if isinstance(best_name, str):
#                                     warnings.warn("Need kwarg \"{}\" to evaluate fully".format(best_name))
...
#                     raise TypeError("Need kwargs ({}) to evaluate".format(", ".join(bad_kwargs)))
#             return self.evalf(*args, **kwargs)
#             # raise NotImplementedError("Cannot evaluate \"{}\" with the given kwargs.".format(self.method))
#         raise NotImplementedError("Cannot evaluate \"{}\"".format(self.method))
#
...
#         return exp(_to_exp(inputs), method=method, mtype="binf", **kwargs)
#
#     @staticmethod
#     def mbinary(inputs, method, **kwargs):
#         """ a f b notation and a f b latex"""
#         return exp(_to_exp(inputs), method=method, mtype="binm", **kwargs)
#
#     @staticmethod
#     def sbinary(inputs, method, **kwargs):
#         """ a f b notation and f{a}{b} latex"""
#         return exp(_to_exp(inputs), method=method, mtype="bins", **kwargs)
#
#     @staticmethod
#     def comp(inputs, method, **kwargs):
#         """ a comp b notation and a comp b latex"""
#         return exp(_to_exp(inputs), method=method, mtype="binr", **kwargs)
#
#     @staticmethod
...
#             input = input[0]
#         return exp(_to_exp(input), method=method, mtype="unaf", **kwargs)
#
#     @staticmethod
#     def sunary(input, method, evalf=None):
#         """ f(a) notation and f{a} latex """
#         if isinstance(input, tuple):
#             input = input[0]
#         return exp(_to_exp(input), method=method, mtype="unas", evalf=evalf)
#
#     # binary operators
...
#                 return sum(l)
#             print(is_float(fro), is_float(to))
#             raise TypeError("Cannot calculate sum from \"{}\" to \"{}\"".format(fro, to))
#         return exp.binary((vari, fro, to, expr), "sigma", evalf=evalf, ignores_vars=True)
#
#     @staticmethod
